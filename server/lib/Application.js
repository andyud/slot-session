...
_.extend(Application.prototype, Base.prototype, {
    ...
    mergeResult : function (result,resultData) {
        result.statusCode = 0;
    ...
    setSuccess : function (result,resultData) {
        result.statusCode = 0;
        if (typeof resultData !== 'undefined') {
            for (let i in resultData) {
                result[i] = resultData[i];
    ...
    checkParameter : function (rParams,body) {
        let params = Object.fromEntries(rParams.map(e => [e, undefined]));
        ...
        if(Object.values(params).every(x => !(x === null  || x === undefined || x === '')))
            return params;


        throw new Error("parameter");
    },
    errorProcess : function (e,result) {


        let error = Garam.getCtl('error').getError(e.message);
