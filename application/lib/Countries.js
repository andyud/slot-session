...
const Africa = [
    'BF', 'DJ', 'BI', 'BJ', 'ZA', 'BW', 'DZ',
    'ET', 'RW', 'TZ', 'GQ', 'NA', 'NE', 'NG',
    'TN', 'LR', 'LS', 'ZW', 'TG', 'TD', 'ER',
    'LY', 'GW', 'ZM', 'CI', 'EH', 'CM', 'EG',
    'SL', 'CG', 'CF', 'AO', 'CD', 'GA', 'GN',
    'GM', 'XS', 'CV', 'GH', 'SZ', 'MG', 'MA',
    'KE', 'SS', 'ML', 'KM', 'ST', 'MW', 'SO',
    'SN', 'MR', 'UG', 'SD', 'MZ'
]
...
Countries.prototype = {
    create : function () {
        ...
        for (let i in Asia) {
            let name = Asia[i];
            this._countries[name] = 'asia';
        }
        for (let i in Europe) {
            let name = Europe[i];
            this._countries[name] = 'europe';
        }
        for (let i in SouthAmerica) {
            let name = SouthAmerica[i];
            this._countries[name] = 'southAmerica';
        }

        for (let i in Africa) {
            let name = Africa[i];
            this._countries[name] = 'africa';
        }


        for (let i in NorthAmerica) {
            let name = NorthAmerica[i];
            this._countries[name] = 'america';
        }
        ...
        this._currentRegion = this.getIpRegion(Garam.get('serverIP'));

        //console.log('#_currentRegion',this._currentRegion)

    },
    getIpRegion : function (ip) {
        function pad(n) {
            return (n.length < 3) ? pad('0' + n) : n;
        }
        function atoi(ip) {
            return parseInt(ip.split('.').map(function (el) { return pad(el); }).join(''), 10);
        }
        ...
        for (let i =0; i < this._regionList.length; i++) {
            ...
            if (inRange(ip, this._regionList[i].startIp, this._regionList[i].lastIp)) {
                return this._regionList[i].region;
            }
        }
    ...
    getContinent : function (country) {
        if (typeof this._countries[country] === 'undefined') {
            return 'america';
        }
