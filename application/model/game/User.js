...
let DP = BaseProcedure.extend({
    ...
    async getMission(userId) {
        let query ="SELECT M.game_id,mission_type,M.ticket,M.role,M.limit,M.stage,U.remain,U.id as mission_id ,U.complete \n" +
            "FROM mission M JOIN user_mission_list U ON M.id = U.mission_id WHERE U.user_id =? AND U.complete = -1";
        return await this.executeAsync(query, [userId], this._read, this._debug);
    },
    async useTicket(userId) {
        let result = await this.executeAsync("SELECT ticket FROM user_info WHERE user_id = ?", [userId], this._read, this._debug);
        if (result.length !== 1)
            throw new Error('ticketUseError');

        if (result[0].ticket < 0) {
            throw new Error('ticketUseError');
        }

        await this.executeAsync("UPDATE user_info SET ticket = ticket -1 WHERE user_id =?",[userId], this._read, this._debug);

        return result[0].ticket - 1;
    },
    async createUser(app,token) {
        ...
        try {
            ...
            query = "SELECT joinBonus FROM bonus_setting where id = 1 ";
            let rows = await this.queryAsync(connection, query,[] , this._write, this._debug);
            query = "INSERT INTO user_info (user_id, balance) VALUES (?, ?)";
            await this.queryAsync(connection, query,[rs.insertId, rows[0].joinBonus] , this._write, this._debug);

            query = "SELECT * FROM slot.mission where stage = 1";
            let missionList = await this.queryAsync(connection, query,[] , this._write, this._debug);
            for await (let mission of missionList) {
                let query ="INSERT INTO user_mission_list SET mission_id =? ,remain =? ,user_id=? ,complete =-1";
                await this.executeAsync(query,[mission.id,mission.role,rs.insertId],this._read);
            }
    ...
    async getUserData(appIndex,token) {
        try {
            let query ="SELECT u.user_id,u.token,u.nick_name,u.nick_name,i.balance,i.level,i.last_lineBet,i.ticket FROM user u left join user_info i ON u.user_id = i.user_id WHERE u.token =?  AND u.app =?"
